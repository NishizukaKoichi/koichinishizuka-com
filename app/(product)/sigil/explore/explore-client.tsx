...
export default function SigilExploreClient({ initialCategory }: { initialCategory: string }) {
  const { t } = useI18n()
  const { userId } = useAuth()
  const router = useRouter()
  const pathname = usePathname()

  const safeInitialCategory = isValidCategory(initialCategory) ? initialCategory : "all"
  ...
  useEffect(() => {
    setSelectedCategory(safeInitialCategory)
  }, [safeInitialCategory])
  ...
  }, [specs, searchQuery, selectedCategory])

  const handleCategorySelect = (categoryId: string) => {
    setSelectedCategory(categoryId)
    const href = categoryId === "all" ? pathname : `${pathname}?category=${encodeURIComponent(categoryId)}`
    router.replace(href)
  }
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    <Button size="sm" onClick={() => handleAdoptClick(spec)} disabled={isAdopted || !userId}>
      ...
      <Dialog open={adoptDialogOpen} onOpenChange={setAdoptDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>採用すると、あなたのアカウントに採用記録が残ります。</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setAdoptDialogOpen(false)}>
              キャンセル
            </Button>
            <Button onClick={handleAdoptConfirm}>採用する</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
